...
class PaintingDB 
{  
    private static $baseSQL = "SELECT PaintingID, Paintings.ArtistID AS ArtistID, FirstName, LastName, GalleryID, ImageFileName, Title, ShapeID, MuseumLink, AccessionNumber, CopyrightText, Description, Excerpt, YearOfWork, Width, Height, Medium, Cost, MSRP, GoogleLink, GoogleDescription, WikiLink FROM Artists INNER JOIN Paintings ON Artists.ArtistID = Paintings.ArtistID ";
    ...
    {
        $sql = self::$baseSQL . " WHERE GalleryID=?" . self::$constraint;
        $statement = DatabaseHelper::runQuery($this->pdo, $sql, Array($galleryID));
        return $statement->fetchAll();        
    } 
    
    public function getAllForShape($shapeID)
    {
    ...
    public function getAllForGenre($genreID)
    {
